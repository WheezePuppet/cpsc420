...
# Parameter sweep of Schelling's Segregation simulation -- version 1.0
...
# Main simulation.
results <- foreach(thresh=seq(0,1,.01),.combine=rbind) %dopar% {
    ...
    return (data.frame(thresh=thresh,avgRatio=computeAvgRatio(city[,,numGen])))
}

plot(results$thresh, results$avgRatio, type="l",
    main="Segregation vs. bigotry", xlab="bigotry (threshold for moving)",
    ylab="segregation (% of neighbors like myself)")
