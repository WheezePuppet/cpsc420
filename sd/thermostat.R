...
outside <- 39        # degF
ins.loss.rate <- .1  # (degF/hr)/degF
hysteresis.factor <- 5 # degF
...
for (i in 2:length(time)) {
    ...
    discrepancy.outside <- room.temp[i-1] - outside     # degF
...
plot(time,room.temp,type="l",col="red",lwd=2,
    xlab="hrs",ylab=~degree~F,main="Temperature over time",
    ylim=range(all.values))
abline(h=thermostat,lty="dashed",col="black")
abline(h=outside,lty="dotted",lwd=2,col="blue")
